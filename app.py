...
# app.py mein index_string ke andar ye style tag update karein
app.index_string = """
<!DOCTYPE html>
<html>
    <head>
...
def about_page():
    return html.Div(
        [
            ...
            # 🔥 OUTER BACKGROUND CONTAINER (NEW)
            html.Div(
                [

                    html.H1("About Us", className="about-title"),

                    # SAME OLD CARDS (UNCHANGED)
                    html.Div(
                        [
                            html.P(
                ...
                className="about-bg-container"  # ✅ ONLY NEW THING
            )
        ]
    )
...
# MAIN APP LAYOUT
# =========================
# app.layout mein dcc.Loading ko hata dein
app.layout = html.Div(
    id="root",
    children=[
...
def get_stock_price_fig(df, theme):
    ...
    fig.update_layout(
        ...
        # Right margin 60 rakha hai taaki icons andar rahein
        margin=dict(l=40, r=60, t=50, b=40),
        modebar={
            "bgcolor": "rgba(0,0,0,0)",
            "color": "#198754" if theme == "light" else "#c3ff00",
            "orientation": "h",  # Icons horizontal rahenge par compact
        },
        hovermode="x unified",
    )
    return fig
...
def ai_predictor_layout():
    return html.Div(
        [
            dbc.Row(
                [
                    # LEFT PANEL: Inputs aur Buttons (Isme koi change nahi)
                    dbc.Col(
                        [
                            dcc.Input(
                                id="dropdown_tickers",
                    ...
                    # RIGHT PANEL: Yahan humne aapka photo wala loading code dala hai
                    dbc.Col(
                        [
                            # Ticker Name aur Description ke liye chota loading
                            dcc.Loading(
                                type="dot",
                                children=[
                                    html.H3(id="ticker"),
...
def show_forecast_error(store):
    ...
    # ❗ Sirf Forecast ke baad hi error dikhao
...
# =========================
# YE NAYA CODE PASTE KARO (Purane 5 functions hatane ke baad)
# =========================
@app.callback(
...
def update_ai_store(
    ...
    # ✅ valid input
    store.pop("forecast_error", None)
    store["forecast_fig"] = prediction(ticker.upper(), n_days + 1, theme).to_dict()
...
def restore_all_ai_ui(pathname, store):
    # Agar path AI Predictor nahi hai to kuch mat karo
    if pathname != "/AI-Predictor":
        raise PreventUpdate

    # Default values initialize karna zaroori hai (Error fix karne ke liye)
    desc, name, g_stk, g_ind, g_frc = "", "", "", "", ""

    # Agar store mein data hai tabhi UI restore karo
    if store:
        desc = store.get("desc", "")
        name = store.get("name", "")

        # --- COMPACT GRAPH CONFIG ---
        # Isse icons chote rahenge aur graph ke andar fit aayenge
        graph_config = {
            "displayModeBar": True,
            "displaylogo": False,
        ...
        # Store se figures nikal kar dcc.Graph mein convert karna
    ...
    # Sabhi outputs ko return kar rahe hain
